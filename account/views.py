...
def dashboard(request):
    ...
    if request.user.is_authenticated:
        ...
        if Company.objects.filter(userID=request.user).exists():
            ...
            rel1 = Post.objects.filter(type=2).filter(b2b_type=1).exclude(soft_delete=True).exclude(userID=request.user)[0:10]
            rel2 = Post.objects.filter(type=2).filter(b2b_type=2).exclude(soft_delete=True).exclude(userID=request.user)[0:10]
            rel3 = Post.objects.filter(type=2).filter(b2b_type=3).exclude(soft_delete=True).exclude(userID=request.user)[0:10]
